...
class EnvRenderer(pyglet.window.Window):
    ...
    def update_map(self, map_path, map_ext):
        ...
        for i in range(map_points.shape[0]):
            map_verts = self.batch.add(1, GL_POINTS, None,
    ...
    def on_draw(self):
        ...
        The function draws the batch created that includes the map points, the agent polygons, and the information
    ...
    def update_obs(self, obs):
        ...
        self.score_label.text = f'Lap Time: {obs["lap_times"][0].round(2)}, ' \
                                f'Ego Lap Count: {obs["lap_counts"][obs["ego_idx"][0]]}'
